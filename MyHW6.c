/*
* Program to simulate the lc3 part II.
*
* Author: Jorie Fe Fernandez
* TCSS 371 - Assignment #6
* 9 December 2016
*/
...
// JSR bit 11
#define JIMMBIT(instr)  ((instr & 0x0800) >> 11)
// offset 11
#define PCOFFSET11(instr) ((instr << 21) >> 21)
#define TRAPVECTOR(instr) (instr & 0x00FF)
   ...
   *(cptr+1) = temp;
   ...
   while (!halt) {   // one instruction executed on each rep.
      ...
      //  printf("pc = %04hx opcode = %02hx\n", pc, opcode);
       ...
       // increment pc
        pc++; 
        int16_t dest_reg, src_reg, imm_bit, imm_val, pcoffset9, src_reg2, pcoffset6, temp, dsr;
        ...
        switch(opcode) {
            ...
            case 10:
                  ...
                  if ((unsigned short)(memory[pc+pcoffset9]) == (unsigned short)DSR) {
                      
                      memory[ (unsigned short) memory[pc+pcoffset9]] = 0x8000;
                      regs[dest_reg] = 0x8000; 
            ...
            case 11:
                  ...
                  memory[(unsigned short) memory[pc+pcoffset9]] = regs[dest_reg];
                  //DDR function to display characters
                  if ((unsigned short)(memory[pc+pcoffset9]) == (unsigned short)DDR) {
                      
                      printf("%c", memory[(unsigned short) memory[pc+pcoffset9]] );
                  }                      
                  break;  
...
static void print_usage (char *progname)
{	printf ("\nUsage : %s <input file> <input file> <input file> <input file> <input file>\n", progname) ;
	puts ("\n"
		"    Where the five input files will contain\n" 
		"    the instructions to be read. Last file is the main program.\n"
		) ;
